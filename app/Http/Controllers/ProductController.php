...
class ProductController extends Controller
{
    public function fetchDataAmazon(Result $result)
    {
        ...
        if ($response->getStatusCode() === 200) {
            ...
            $track = Track::where('prod_id',$link->prod_id)->first();
            log::info($track);

//            $filename = 'Amazon' . date('Y-m-d_H-i-s') . '.xlsx';
//            Excel::store(new ResultExport, $filename);


//         $employeeEmail = 'yousefbadr10@gmail.com'; // Change this to the employee's email address
//            Mail::to($employeeEmail)->send(new \App\Mail\ResultsExportEmail($filename));
